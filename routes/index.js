...
router.get('/product/:id', isLoggedIn, async (req, res) => {
    ...
    Product.findById(req.params.id, function (e, product) {
        Product_type.findById(product.product_type_id, function (error, product_type) {
            if(error) throw error;
            if (product_type == null) {
                // alert('Loại đồ uống ứng với topping đã bị xoá!\nBạn hãy chỉnh sửa "Đồ uống >> Sửa >> Tên loại đồ uống".Thx :3');
                res.redirect('/');
            } else {
